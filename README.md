...
- **Native DSH experience:** SideChat uses DSH child sessions, transcripts, status, cancellation, timing, token usage, and Web navigation.
- **Bounded lifecycle:** completed SideChats stay visible for 30 minutes, with at most five retained per parent session.
...
The parent composer unlocks immediately. Open the subagent list to watch the SideChat run and read its answer.

Cancel the newest active SideChat, or a specific request:
...
## What can SideChat see?

SideChat receives two immutable inputs:
...
This is a capture-time snapshot. SideChat does not continue following parent events after it starts.
...
- SideChat cannot modify files, run commands, poll later parent activity, or inspect sibling subagents.
...
flowchart LR
    ...
    F --> T[Native SideChat transcript]
...
| `retentionMinutes` | `30` | Minutes a completed SideChat remains visible. |
| `maxRetainedPerParent` | `5` | Completed SideChats retained per direct parent. |
...
- One-shot SideChats only; no follow-up messages inside a completed child.
- No dedicated SideChat panel.
- No parent-bound read-only polling tools.
- No promotion of a SideChat result into the parent conversation.
