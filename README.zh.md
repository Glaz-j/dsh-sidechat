...
> **开发者预览：** 当前版本可以通过 GitHub 或本地目录安装测试。等归档可见性适配进入 DSH 官方发行版后，再发布 npm 正式版本。详见[兼容性](#兼容性)。
...
目前这个包还没有发布到 npm。DSH 并不强制插件上线 npm，但 npm 安装比 Git 依赖的现场构建更简单、也更容易复现。
...
SideChat 完成后会在父会话下保留 30 分钟。每个父会话最多显示最近 5 个已经完成的 SideChat；第 6 个完成时，最旧的一个会立刻归档。正在运行的子 Agent 不受数量规则影响。

归档不会删除数据：记录仍然持久化，只是不再出现在普通的子会话分组界面中。DSH 重启后，启动协调逻辑会根据历史状态恢复归档截止时间。
...
命令、快照、fork、隔离和取消流程使用的是 DSH 公开的插件接缝。目前的归档 UI 行为还要求 DSH 宿主支持：在原生子 Agent 列表与数量统计中隐藏已归档会话。

这个宿主适配已经存在于本项目使用的开发环境中，但尚未进入 DSH 官方 `0.1.0-rc.7` 版本。在它被上游接收并发布以前，GitHub 安装适合开发测试；若要获得完整的保留与归档表现，还需要配套的 DSH 宿主补丁。建议解决这个兼容点之后再发布首个 npm 版本。
